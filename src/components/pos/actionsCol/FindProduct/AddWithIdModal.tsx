...
const AddWithIdModal = ({
  ...
  const cartItemQuantity = cartData?.items?.find(
    (cartItem) => cartItem?.product?.identifier === productItem?.identifier
  )?.quantity;
  ...
  const { mutate: addWithIdentifierMutation, isPending } = useMutation({
    ...
    onSuccess: (res) => {
      if (!localStorage?.getItem("cartIdentifier")) {
        useAxios.interceptors.request.use((config) => {
          config.headers["Cart-Identifier"] = `${res?.data?.cart?.identifier}`;
          return config;
        });
  ...
  const onSubmit = (e: any) => {
    ...
    addWithIdentifierMutation({
      product: productItem?.identifier,
      quantity: cartCount,
    });
  };
