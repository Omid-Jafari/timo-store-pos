import { deleteRefundFunc } from "@/api/ApiClient";
...
const DeleteRefund = ({ refund }: { refund: Refund }) => {
  const [t] = useTranslation("global");
  const [isOpen, setIsOpen] = useState(false);
  const queryClient = useQueryClient();

  const { mutate: deleteRefundMutation, isPending } = useMutation({
    mutationKey: ["deleteRefundMutation"],
    mutationFn: deleteRefundFunc,
    onSuccess: () => {
  ...
  return (
    <ModalComp
      ...
      title={t("refunds.deleteRefund")}
      open={isOpen}
      setOpen={setIsOpen}
      dialogContentClassName="max-w-md"
    >
      <p className="font-medium mb-5">{t("refunds.deleteRefundConfirm")}</p>
      <div className="w-full flex items-center gap-2 justify-end">
        ...
        <button
          disabled={isPending}
          type="button"
          onClick={() => deleteRefundMutation(refund?.identifier)}
