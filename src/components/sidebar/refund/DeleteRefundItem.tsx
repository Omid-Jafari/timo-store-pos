import { deleteRefundItemFunc } from "@/api/ApiClient";
...
const DeleteRefundItem = ({ refundItem }: { refundItem: RefundItem }) => {
  const [t] = useTranslation("global");
  const [isOpen, setIsOpen] = useState(false);
  const queryClient = useQueryClient();

  const { mutate: deleteRefundItemMutation, isPending } = useMutation({
    mutationKey: ["deleteRefundItemMutation"],
    mutationFn: deleteRefundItemFunc,
    onSuccess: () => {
    ...
    >
      ...
      <div className="w-full flex items-center gap-2 justify-end">
        ...
        <button
          disabled={isPending}
          type="button"
          onClick={() => deleteRefundItemMutation(refundItem?.identifier)}
...
export default DeleteRefundItem;
